...
function email(action, data) {
  return new Promise(function (resolve, reject) {
    ...
    // configure AWS SDK
    aws.config.update({ "accessKeyId": 'AKIAIXHD4TILQQGMDFTQ', "secretAccessKey": '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', "region": "eu-west-1" });
...
// send some mail
    transporter.sendMail({
      ...
      if (err == undefined) {
        resolve({state: 'done', message: 'email sent'});
      } else {
        reject(err);
      }
    });
